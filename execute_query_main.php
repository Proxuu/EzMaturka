...
$dzial = isset($_POST['dzial']) ? $conn->real_escape_string($_POST['dzial']) : '';
...
switch ($queryParam) {
    ...
    case 'random_id':
        if (!empty($dzial)) {
            $sql = "SELECT id FROM exercise WHERE dzial=? ORDER BY RAND() LIMIT 1";
            $params = [$dzial];
            $types = 's';
        } else {
            $sql = "SELECT id FROM exercise ORDER BY RAND() LIMIT 1";
        }
        break;
    default:
